...
export default function SkillSearchDetails() {
  ...
  useEffect(() => {
    ...
    const filtered = employees.filter((employee) => {
      ...
      const commonSkills = employeeSkills.filter((skill) => skills.includes(skill));
  ...
  >
    <div
      ...
      style={{ maxWidth: "500px", minHeight: "60px", width: "400px", height: "60px" }}
    >
      <div className="text-center px-4 flex flex-col justify-center h-full">
        <h3 className="text-lg font-semibold mb-4">{employee.employeeid} {employee.name} {employee.skills}</h3>
      </div>
    </div>
    <input
      type="radio"
