...
export default function SkillSearch() {
  ...
  const [checkbox, setCheckBox] = useState(new Array(all_skills.length).fill(false)); //stores the selection status of each box
  ...
  return <div>
          ...
          <div>
            {selectedSkills.map((val,index)=>(
                <p key={index} style={{ display: 'inline' }}>{val}      </p>
            ))}
          </div>
